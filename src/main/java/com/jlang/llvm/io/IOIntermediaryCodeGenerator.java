...
public class IOIntermediaryCodeGenerator implements IOGenerator {
    ...
    public String printf(String id, VariableType variableType) {
        return "%s = load %s, ptr %s ".formatted(REGISTRY_SYMBOL + registry++, variableType.getLlvmVariableNameLiteral(), REGISTRY_SYMBOL + id) +
                "%s = call i32 (ptr, ...) @printf(ptr noundef @.str%s, %s %s".formatted(REGISTRY_SYMBOL + registry++, variableType.getLlvmStringPointer(), variableType.getLlvmVariableNameLiteral(), REGISTRY_SYMBOL + (registry - 2));
    }
    ...
    public String scanf(String id, VariableType variableType) {
        return """
                ...
                """.formatted(REGISTRY_SYMBOL + registry++, variableType.getLlvmStringPointer(), REGISTRY_SYMBOL + id);
    }
}
